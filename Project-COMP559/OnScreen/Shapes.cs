...
namespace Library
{
    public sealed class Shapes : IDisposable
    {
        ...
        public void DrawBoxFill_Slow(Vector2 center, float width, float height, float rotation, Vector2 scale, Color color)
        {
            ...
            Transform transform = new Transform(center, rotation, scale);

            Vector2 a = Util.Transform(new Vector2(left, top), transform);
            Vector2 b = Util.Transform(new Vector2(right, top), transform);
            Vector2 c = Util.Transform(new Vector2(right, bottom), transform);
            Vector2 d = Util.Transform(new Vector2(left, bottom), transform);
        ...
        public void DrawCircleFill(in Circle circle, int points, Color color)
        {
        ...
        public void DrawCircleFill(float x, float y, float radius, int points, Color color)
        {
            ...
            int shapeVertexCount = MathHelper.Clamp(points, MinCirclePoints, MaxCirclePoints);    // The vertex count will just be the number of points requested by the user.
            ...
            float angle = Microsoft.Xna.Framework.MathHelper.TwoPi / (float)shapeVertexCount;
        ...
        public void DrawEllipseFill(Vector2 center, Vector2 radius, int points, Color color)
        {
            ...
            int shapeVertexCount = MathHelper.Clamp(points, MinElipsePoints, MaxElipsePoints);    // The vertex count will just be the number of points requested by the user.
            int shapeTriangleCount = shapeVertexCount - 2;      // The triangle count of a convex polygon is alway 2 less than the vertex count.
            int shapeIndexCount = shapeTriangleCount * 3;       // The indices count will just be 3 times the triangle count.

            this.EnsureSpace(shapeVertexCount, shapeIndexCount);

            float deltaAngle = Microsoft.Xna.Framework.MathHelper.TwoPi / (float)shapeVertexCount;
            float angle = 0f;
        ...
        public void DrawPolygonFill(Vector2[] vertices, int[] triangles, Transform transform, Color color)
        {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                v = Util.Transform(v.X, v.Y, transform);
        ...
        public void DrawPolygonFill(Span<Vector2> vertices, int[] triangles, Transform transform, Color color)
        {
            this.EnsureStarted();
            this.EnsureSpace(vertices.Length, indices.Length);
        ...
        public void DrawLine(float x1, float y1, float x2, float y2, Color color)
        {
            ...
            MathHelper.Normalize(ref e1x, ref e1y);
        ...
        public void DrawCircle(in Circle circle, int points, Color color)
        {
        ...
        public void DrawCircle(float x, float y, float radius, int points, Color color)
        {
            ...
            points = MathHelper.Clamp(points, MinCirclePoints, MaxCirclePoints);

            float angle = Microsoft.Xna.Framework.MathHelper.TwoPi / (float)points;
        ...
        public void DrawEllipse(Vector2 center, Vector2 radius, int points, Color color)
        {
            ...
            float deltaAngle = Microsoft.Xna.Framework.MathHelper.TwoPi / (float)points;
            float angle = 0f;
        ...
        public void DrawPolygon(ReadOnlySpan<Vector2> vertices, Transform transform, Color color)
        {
        ...
        public void DrawPolygonTriangles(Vector2[] vertices, int[] triangles, Transform transform, Color color)
        {
