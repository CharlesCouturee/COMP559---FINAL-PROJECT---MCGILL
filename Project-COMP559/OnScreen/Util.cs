...
namespace Library
{
    public static class Util
    {
        ...
        {
            ratio = MathHelper.Clamp(ratio, 0.25f, 1f);
        ...
        public static Vector2 Transform(Vector2 value, Transform transform)
        {
            return new Vector2(
        ...
        public static bool IntersectBoxes(Box a, Box b)
        {
            if (a.Max.X <= b.Min.X ||
                b.Max.X <= a.Min.X ||
                a.Max.Y <= b.Min.Y ||
        ...
        public static bool IntersectBoxes(Box a, Box b, out Vector2 mtv)
        {
            mtv = Vector2.Zero;

            if (!Util.IntersectBoxes(a, b))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d, out Vector2 ip)
        {
            ...
            if (!Util.IntersectSegmentsFast(a.X, a.Y, b.X, b.Y, c.X, c.Y, d.X, d.Y))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d)
        {
            ...
            float c1 = MathHelper.Cross(ab, ac);
            float c2 = MathHelper.Cross(ab, ad);

            float c3 = MathHelper.Cross(cd, ca);
            float c4 = MathHelper.Cross(cd, cb);
        ...
        public static void CreateBoundingBox(Vector2[] vertices, int[] hull, out Box box)
        {
            ...
            box = new Box(xmin, xmax, ymin, ymax);
        }

        public static void CreateBoundingBox(Vector2[] vertices, out Box box)
        {
            float xmin = float.MaxValue;
            float xmax = float.MinValue;
            float ymin = float.MaxValue;
        ...
        public static bool IsPointInTriangle(Vector2 p, Vector2 a, Vector2 b, Vector2 c)
        {
            ...
            float c1 = MathHelper.Cross(ab, ap);
            float c2 = MathHelper.Cross(bc, bp);
            float c3 = MathHelper.Cross(ca, cp);
        ...
        {
            return MathF.Abs(a - b) <= Util.VerySmallAmount;
        }

        public static bool IsNearlyEqual(Vector2 a, Vector2 b)
        {
            return
                (MathF.Abs(a.X - b.X) <= Util.VerySmallAmount) &&
                (MathF.Abs(a.Y - b.Y) <= Util.VerySmallAmount);
        }
    }
}
